...
//Graph generated in SVG format
	...
	function drawText($x, $y, $element) {
		...
		if ($element->children) {
			...
			$a->setAttribute("xlink:href", $_SERVER['PHP_SELF']."?family=$family&".$f."c=".$element->name."&svg=yes");
			$a->appendChild($text);
			$g->appendChild($a);
		} else {
	...
	function drawNavBar($name) {
		...
		if ($myDoc[0]->getParent($name)) {
			$a = $doc->createElement("a");
			$a->setAttribute("xlink:href", $_SERVER['PHP_SELF']."?family=$family&".$f."c=".$myDoc[0]->getParent($name)->getAttribute("name")."&svg=yes");
			$text = $doc->createElement("text");
